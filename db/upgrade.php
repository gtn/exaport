...
function xmldb_block_exaport_upgrade($oldversion) {
    ...
    if ($oldversion < 2026021801) {
        ...
        // Foreign keys automatically create indexes, so no separate index needed on courseid.
        $table->add_key('courseid', XMLDB_KEY_FOREIGN, array('courseid'), 'course', array('id'));

        // Only add index for pid (non-foreign-key field).
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        // Foreign-unique key creates unique index automatically, no separate index needed.
    ...
    if ($oldversion < 2026052903) {
        ...
        // Exaport savepoint reached.
        upgrade_block_savepoint(true, 2026052903, 'exaport');
    }
    ...
    //     // }
    //
    //     upgrade_block_savepoint(true, 2026052902, 'exaport');
    // }

    if ($oldversion < 2026060300) {
        // Add notify field to block_exaportcatshar table.
        $table = new xmldb_table('block_exaportcatshar');
        $field = new xmldb_field('notify', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'userid');
    ...
    if ($oldversion < 2026062903) {
        ...
        // Exaport savepoint reached.
        upgrade_block_savepoint(true, 2026062903, 'exaport');
    }
    ...
    if ($oldversion < 2026080700) {
        ...
        // item sharing can be wired into the access-check functions.
        $table = new xmldb_table('block_exaportitemshar');

        $field = new xmldb_field('notify', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'courseid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('timestart', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'notify');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        // Create block_exaportitemgroupshar (item -> group/cohort shares).
        $grouptable = new xmldb_table('block_exaportitemgroupshar');
        $grouptable->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $grouptable->add_field('itemid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $grouptable->add_field('groupid', XMLDB_TYPE_INTEGER, '20', null, XMLDB_NOTNULL, null, '0');
