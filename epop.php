...
if ($action=="login"){
	...
	if ($uname!="0" && $pword!="0"){
		...
		if (!$user = $DB->get_record("user", $conditions)){
			...
			$validiert=true;//alte version bei der die passw�r verschl� geschickt werden
		}
	...
	if (!$user) echo "invalid hash";
		...
		//die kompetenzen werden nach subject gruppiert angezeigt, daher nur diese gruppe l�en
	...
	if (!$user) echo "invalid hash";
		...
		if ($itemid>0){
				...
				if (!empty($itemrs)){
					/* 
					//auch �s items k�n nicht aktualisiert werden, die m�gel�t werden, ab 31.5.13
					if ($itemrs->isoez==1){ //normale items k�n files nicht aktualisiert werden, da muss das ganze item gel�t werden
						$sql="SELECT f.* FROM {block_exaportitem} i INNER JOIN {files} f ON i.id=f.itemid
		...
		if ($action=="updatePic"){
			...
			if ($itemrs->isoez!=1){ //wenn neues item, �s items k�n eh nicht neu sein
				$new->userid = $user->id;
			...
			if ($action!="updatePic"){
				if($itemid>0){
			           		...
			           		if ($itemrs->isoez!=1){
					          	...
					          	block_exaport_delete_competences($itemid,$user->id);
					          	//wenn text oder link, dann beispiel gel� wenn type file ist datei dabei, auch gel�
...
function write_xml_items($conditions,$view_id=0,$competence_category=""){
	...
	if ($items = $DB->get_records("block_exaportitem", $conditions, " isoez DESC,name")){
			...
			foreach($items as $item){
				...
				/*itemauswahl f�w: nur gel� aufgaben/items anzeigen*/
				if ($view_id>0){
...
function kuerzen($wert,$laenge){
	if (strlen($wert)>$laenge){
		$wert = substr($wert, 0,$laenge ); // gibt "abcd" zur�	}
	return $wert;
}
...
function block_exaport_installoez($userid,$isupdate=false){
	...
	if ($isupdate==true){
		...
		//dann neue daten durchlaufen, wenn neu dann insert, wenn vorhanden dann title und parentid pr�nd bei bedarf update, f�chen merker machen
		//echo $userid;die;
