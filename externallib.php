...
class block_exaport_external extends external_api {
	...
	public static function set_item_competence($itemid, $descriptorid, $val) {
		...
		if ($val == 1) {
			...
			$DB->insert_record("block_exacompcompactiv_mm", array('compid' => $descriptorid, 'activityid' => $itemid, 'eportfolioitem' => 1, 'activitytitle' => $item->name, 'coursetitle' => $course->shortname, 'comptype' => BLOCK_EXACOMP_TYPE_DESCRIPTOR));
			$DB->insert_record('block_exacompcompuser_mm', array("compid" => $descriptorid, "activityid" => $itemid, "eportfolioitem" => 1, "reviewerid" => $USER->id, "userid" => $USER->id, "role" => 0, 'comptype' => BLOCK_EXACOMP_TYPE_DESCRIPTOR));
		} else if ($val == 0) {
			$DB->delete_records("block_exacompcompactiv_mm", array('compid' => $descriptorid, 'activityid' => $itemid, 'eportfolioitem' => 1, 'comptype' => BLOCK_EXACOMP_TYPE_DESCRIPTOR));
			$DB->delete_records("block_exacompcompuser_mm", array("compid" => $descriptorid, 'activityid' => $itemid, 'eportfolioitem' => 1, 'comptype' => BLOCK_EXACOMP_TYPE_DESCRIPTOR));
		}
	...
	public static function get_competencies_by_item($itemid) {
		...
		return $DB->get_records("block_exacompcompactiv_mm", array("activityid" => $itemid, "eportfolioitem" => 1, "comptype" => BLOCK_EXACOMP_TYPE_DESCRIPTOR), "", "compid as competenceid");
	}
