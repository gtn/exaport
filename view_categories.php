		...
		switch ( $cataction )
		{
			...
			case "edit":
				...
				if(($editid > 0) && ($editrecord = $DB->get_record("block_exaportcate", $conditions)))
				{
					...
					?><div class="block_eportfolio_center"><form method="post" action="<?php echo $CFG->wwwroot; ?>/blocks/exaport/view_categories.php?courseid=<?php echo $courseid; ?>&amp;edit=1">
					  <fieldset>
...
if($edit == 1) {
	...
	$outer_categories = $DB->get_records_select("block_exaportcate","userid = ? AND pid = ?",$conditions, "name asc");
	if ( $outer_categories ) {
