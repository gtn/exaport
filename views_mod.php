...
$query = "select i.id, i.name, i.type, i.url AS link, ic.name AS cname, ic2.name AS cname_parent, COUNT(com.id) As comments, files.mimetype as mimetype".
	 " from {block_exaportitem} i".
	 " join {block_exaportcate} ic on i.categoryid = ic.id".
	 " left join {block_exaportcate} ic2 on ic.pid = ic2.id".
	 " left join {block_exaportitemcomm} com on com.itemid = i.id".
	 " left join {files} files on files.itemid = i.id".
	 " where i.userid=?".
...
switch ($type) {
	case 'content' :
	// view data form
echo '<div id="blocktype-list">Drag and drop content blocks from the tabs below to create your page.';
echo '<ul>
    <li class="portfolioElement" title="Personal information" block-type="personal_information">
        <div class="blocktype" style="position: relative;">
            ...
            <h4 class="blocktype-title js-hidden">Personal information</h4>
            <div class="blocktype-description js-hidden">Personal information</div>
        </div>
    </li>
    <li class="portfolioElement" title="Headertext" block-type="headline">
        <div class="blocktype" style="position: relative;">
            <img width="73" height="61" alt="Preview" src="'.$CFG->wwwroot.'/blocks/exaport/pix/header_text.png">
            <h4 class="blocktype-title js-hidden">Headertext</h4>
            <div class="blocktype-description js-hidden">Headertext</div>
        </div>
    </li>
    <li class="portfolioElement" title="Text" block-type="text">
        <div class="blocktype" style="position: relative;">
            <img width="73" height="61" alt="Preview" src="'.$CFG->wwwroot.'/blocks/exaport/pix/text.png">
            <h4 class="blocktype-title js-hidden">Text</h4>
            <div class="blocktype-description js-hidden">Text</div>
        </div>
    </li>
    <li class="portfolioElement" title="Items" block-type="item">
        <div class="blocktype" style="position: relative;">
            <img width="73" height="61" alt="Preview" src="'.$CFG->wwwroot.'/blocks/exaport/pix/lists.png">
            <h4 class="blocktype-title js-hidden">Items</h4>
            <div class="blocktype-description js-hidden">Select items</div>
        </div>
    </li>	
</ul>';
echo '</div>';
...
echo '<div class="view-middle">';
	...
	echo '<div id="view-preview">';
		...
		echo '<div>';
			echo '<table cellspacing="0" cellpadding="0" width="100%" class="layout'.$view->layout.'"><tr>';
			for ($i=1; $i<=$cols_layout[$view->layout]; $i++) {
				echo '<td valign="top" class="td'.$i.'">';
...
// --------------------		
	case 'layout' :
			echo '
			<p>Select how you would like the columns in your page to be laid out.</p>
...
echo '<div id="block_form" class="block" style="position: absolute; top: 10px; left: 30%; width: 510px;">
        <div class="block-controls">                
            <a class="delete" title="delete"  href="#"><img src="'.$CFG->wwwroot.'/blocks/exaport/pix/remove-block.png"></a>
        </div>
        <div class="block-header">
            <h4>Block: configure:</h4>
        </div>
        <div class="block-content">
        	<form enctype="multipart/form-data" id="blockform" action="" method="post" name="blockform" class="pieform">
